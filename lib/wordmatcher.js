...
export function fetchKeywords(resume, jobDesc){
  findKeywords(resume, "resume-keywords");
  findKeywords(jobDesc, "jd-keywords");
}
...
export function findKeywords(parsedInfo,divId) {
  ...
  Algorithmia.client("simCnX5SgvLeu7IMGCGgT0GWnE41")
       ...
       .then(function(output) {
         ...
         if (divId === "resume-keywords") {
           window.resumeKeywords = sortedKeywords;
         } else{
           window.jobKeywords = sortedKeywords;
         }
         modifyKeywords(sortedKeywords, divId);
         if (window.jobKeywords && window.resumeKeywords) {
           suggestedKeywords(window.resumeKeywords, window.jobKeywords);
         }
       });
}
...
function modifyKeywords(keywords, divId) {
  ...
  document.getElementsByClassName('keywords-container')[0].style.display = "inline-block";
}

function suggestedKeywords(resumeKeywords, jdKeywords) {
  ...
  document.getElementById('suggestion-1').innerHTML = `For ${jdKeywords[0]}, you ${keywordSuggestions[0][0]}, ${keywordSuggestions[0][1]}, ${keywordSuggestions[0][2]}`;
  document.getElementById('suggestion-2').innerHTML = `For ${jdKeywords[1]}, you ${keywordSuggestions[1][0]}, ${keywordSuggestions[1][1]}, ${keywordSuggestions[1][2]}`;
  document.getElementById('suggestion-3').innerHTML = `For ${jdKeywords[2]}, you ${keywordSuggestions[2][0]}, ${keywordSuggestions[2][1]}, ${keywordSuggestions[2][2]}`;
  document.getElementsByClassName('suggestions-container')[0].style.display = "block";
}
